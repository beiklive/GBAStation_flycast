...
static void gui_display_content()
{
	...
	ImGui::BeginChild(ImGui::GetID("library"), ImVec2(0, 0), ImGuiChildFlags_Border, ImGuiWindowFlags_DragScrolling | ImGuiWindowFlags_NavFlattened);
    {
		...
		if (gameListEmpty && gui_state != GuiState::SelectDisk)
		{
			...
			// center horizontally
			const float w = largeFont->CalcTextSizeA(ImFontLegacySize(largeFont), FLT_MAX, -1.f, label).x + ImGui::GetStyle().FramePadding.x * 2;
	...
	if (!achievements::notifier.draw())
		...
		if (!toast.draw())
		{
			...
			if (!message.empty())
			{
				...
				const ImVec2 size = largeFont->CalcTextSizeA(ImFontLegacySize(largeFont), FLT_MAX, maxW, &message.front(), &message.back() + 1)
						+ padding * 2.f;
				...
				dl->AddText(largeFont, ImFontLegacySize(largeFont), pos, col, &message.front(), &message.back() + 1, maxW);
			}
		}
