...
// Libretro negotiates the VkDevice before it invokes context_reset(), so no
// VulkanContext exists while VkCreateDevice() runs. Preserve these values
// until init() receives the negotiated device.
struct PendingDeviceCapabilities {
...
PendingDeviceCapabilities pending_device_capabilities;
...
{
	NOTICE_LOG(RENDERER, "VkCreateDevice stage=enter context=%p instance=%p gpu=%p surface=%p gipa=%p",
			context, instance, gpu, surface, reinterpret_cast<void*>(get_instance_proc_addr));
	if (context == nullptr || instance == VK_NULL_HANDLE || get_instance_proc_addr == nullptr)
	{
		ERROR_LOG(RENDERER, "VkCreateDevice received an incomplete libretro Vulkan context");
		return false;
	}
#if VULKAN_HPP_DISPATCH_LOADER_DYNAMIC == 1
	NOTICE_LOG(RENDERER, "VkCreateDevice stage=dispatcher-global");
	VULKAN_HPP_DEFAULT_DISPATCHER.init(get_instance_proc_addr);
	NOTICE_LOG(RENDERER, "VkCreateDevice stage=dispatcher-instance");
	VULKAN_HPP_DEFAULT_DISPATCHER.init(vk::Instance(instance));
	NOTICE_LOG(RENDERER, "VkCreateDevice stage=dispatcher-ready");
#endif
	...
	if (gpu == VK_NULL_HANDLE)
	{
		...
		NOTICE_LOG(RENDERER, "VkCreateDevice stage=enumerate-physical-devices");
		...
		NOTICE_LOG(RENDERER, "VkCreateDevice stage=enumerate-proc ptr=%p",
			reinterpret_cast<void*>(enumerate_physical_devices));
		if (enumerate_physical_devices == nullptr)
		{
			ERROR_LOG(RENDERER, "VkCreateDevice could not resolve vkEnumeratePhysicalDevices");
			return false;
		}
		uint32_t device_count = 0;
		VkResult enumerate_result = enumerate_physical_devices(instance, &device_count, nullptr);
		NOTICE_LOG(RENDERER, "VkCreateDevice stage=enumerate-count result=%d count=%u",
			(int)enumerate_result, device_count);
		if (enumerate_result != VK_SUCCESS || device_count == 0)
		{
			ERROR_LOG(RENDERER, "VkCreateDevice physical device enumeration failed result=%d", (int)enumerate_result);
			return false;
		}
		std::vector<VkPhysicalDevice> raw_devices(device_count);
		enumerate_result = enumerate_physical_devices(instance, &device_count, raw_devices.data());
		NOTICE_LOG(RENDERER, "VkCreateDevice stage=enumerate-list result=%d count=%u",
			(int)enumerate_result, device_count);
		if (enumerate_result != VK_SUCCESS)
		{
			ERROR_LOG(RENDERER, "VkCreateDevice physical device list failed result=%d", (int)enumerate_result);
			return false;
		}
		std::vector<vk::PhysicalDevice> devices;
		devices.reserve(device_count);
		for (uint32_t index = 0; index < device_count; ++index)
			devices.emplace_back(raw_devices[index]);
		NOTICE_LOG(RENDERER, "VkCreateDevice stage=enumerate-complete count=%u", (unsigned)devices.size());
		if (devices.empty())
		{
			ERROR_LOG(RENDERER, "VkCreateDevice found no physical devices");
			return false;
		}
		for (const auto& phyDev : devices)
		{
			vk::PhysicalDeviceProperties props;
			NOTICE_LOG(RENDERER, "VkCreateDevice stage=get-properties-candidate");
	...
	NOTICE_LOG(RENDERER, "VkCreateDevice stage=get-properties-selected");
	const vk::PhysicalDeviceProperties physicalDeviceProperties = physicalDevice.getProperties();

	NOTICE_LOG(RENDERER, "VkCreateDevice stage=get-queue-families");
	std::vector<vk::QueueFamilyProperties> queueFamilyProperties = physicalDevice.getQueueFamilyProperties();
	NOTICE_LOG(RENDERER, "VkCreateDevice stage=get-queue-families-complete count=%u",
			(unsigned)queueFamilyProperties.size());
	...
	{
		NOTICE_LOG(RENDERER, "VkCreateDevice stage=get-surface-support-primary");
	...
	NOTICE_LOG(RENDERER, "VkCreateDevice stage=enumerate-device-extensions");
	const auto deviceExtensionProperties = physicalDevice.enumerateDeviceExtensionProperties();
	NOTICE_LOG(RENDERER, "VkCreateDevice stage=enumerate-device-extensions-complete count=%u",
			(unsigned)deviceExtensionProperties.size());
	...
	{
		NOTICE_LOG(RENDERER, "VkCreateDevice stage=get-features2");
		physicalDevice.getFeatures2(&featuresChain);
	}
	else
	{
		NOTICE_LOG(RENDERER, "VkCreateDevice stage=get-features");
		physicalDevice.getFeatures(&features);
	}
	NOTICE_LOG(RENDERER, "VkCreateDevice stage=get-features-complete");
	...
	pending_device_capabilities = {
		.dedicated_allocation = dedicatedAllocationSupported,
		.provoking_vertex = provokingVertexSupported,
	...
	NOTICE_LOG(RENDERER, "VkCreateDevice stage=create-device queues=%u extensions=%u",
			singleQueue ? 1u : 2u, (unsigned)enabledExtensions.size());
	vk::Device newDevice = physicalDevice.createDevice(deviceCreateInfo);
	NOTICE_LOG(RENDERER, "VkCreateDevice stage=create-device-complete device=%p", (VkDevice)newDevice);
	...
	// Queues
	NOTICE_LOG(RENDERER, "VkCreateDevice stage=get-queues");
	...
	NOTICE_LOG(RENDERER, "VkCreateDevice stage=complete queue=%p present=%p", context->queue,
...
bool VulkanContext::init(retro_hw_render_interface_vulkan *retro_render_if)
{
	...
	dedicatedAllocationSupported = pending_device_capabilities.dedicated_allocation;
	provokingVertexSupported = pending_device_capabilities.provoking_vertex;
	samplerAnisotropy = pending_device_capabilities.sampler_anisotropy;
	fragmentStoresAndAtomics = pending_device_capabilities.fragment_stores_and_atomics;
