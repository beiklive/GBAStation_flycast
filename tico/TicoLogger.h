...
public:
  ...
  void Log(Level level, const char *category, uint32_t flags,
    ...
    if (!m_file) {
      ...
      m_file = fopen("sdmc:/GBAStation/debug/flycast_stub.log", "a");
    }
    if (!m_file)
      return;
...
#ifdef __SWITCH__
  bool OpenFile(const char *path, bool truncate) {
    if (m_file) {
      fclose(m_file);
      m_file = nullptr;
    }
    mkdir("sdmc:/GBAStation", 0777);
    mkdir("sdmc:/GBAStation/debug", 0777);
    m_logPath = path ? path : "sdmc:/GBAStation/debug/flycast_stub.log";
    m_file = fopen(m_logPath.c_str(), truncate ? "w" : "a");
...
#ifdef __SWITCH__
    m_file = nullptr;
    m_logPath = "sdmc:/GBAStation/debug/flycast_stub.log";
#endif
